...
typedef struct
{
    ...
    void *pItemsExData[COT_MENU_MAX_NUM];   /*!< 当前菜单中所有选项注册时的扩展数据 */
} cotMenuShow_t;
...
typedef struct
{
    const char        *(pszDesc[COT_MENU_SUPPORT_LANGUAGE]);/*!< 当前选项的字符串描述(多语种) */
...
typedef struct
{
    ...
    cotMenuCallFun_f   pfnEnterCallFun;    /*!< 主前菜单进入时(进入菜单)需要执行一次的函数, 为NULL不执行 */

    cotMenuCallFun_f   pfnExitCallFun;     /*!< 主前菜单进入后退出时(退出菜单)需要执行一次的函数, 为NULL不执行 */

    cotMenuCallFun_f   pfnLoadCallFun;     /*!< 主菜单每次加载时需要执行一次的函数, 为NULL不执行 */
...
extern int cotMenu_Bind(cotMenuList_t *pMenuList, menusize_t menuNum, cotShowcotMenuCallFun_f pfnShowMenuFun);
